...
(defun add-log-entry ()
  ...
  (let ((callsign (read-string "Callsign: "))
        ...
        (date (read-string "Date (YYYYMMDD): "))
        (time (read-string "UTC (HHMM): "))
  ...
  (with-temp-buffer
    ...
    (while (not (eobp))
      (let ((callsign (progn (search-forward "Callsign: ") (thing-at-point 'word)))
            ...
            (date (progn (search-forward "Date (YYYYMMDD): ") (thing-at-point 'word)))
            (time (progn (search-forward "UTC (HHMM): ") (thing-at-point 'word)))
